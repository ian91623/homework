...
VecIterator< T > vector< T >::insert( iterator position, const T val )
{
	...
	int location = 0;
	for(auto it = begin(); it != position; ++it, ++location);
	if(capacity() == size())
	{
		T* tempPtr = new T[capacity() + 10];
		for (int i = 0; i < location; i++)
			tempPtr[i] = myFirst[i];
		tempPtr[location] = val;
		for (int i = location; i < size(); i++)
			tempPtr[i + 1] = myFirst[i];
		myEnd = tempPtr + capacity() + 10;
		myLast = tempPtr + size() + 1;
		delete[] myFirst;
		myFirst = tempPtr;
	}
	else
