...
public:
   ...
   bool operator>( const HugeInteger< T > &right ) const; //addition, greater than
   ...
   void convert( T vector ); // convert a vector of decimal digits into a HugeInteger
...
HugeInteger< T > HugeInteger< T >::operator/( const HugeInteger< T > &op2 ) const
{
   ...
   HugeInteger quotient< T >(integer.size() - op2.integer.size() + 1);
...
HugeInteger< T > HugeInteger< T >::operator%( const HugeInteger< T > &op2 ) const
{
   ...
   HugeInteger< T > remainder = ( *this ) - ( quotient * op2 );
   return remainder;
}
...
void HugeInteger< T >::convert( T v )
{
   ...
   T::iterator it1 = integer.end() - 1;
   T::iterator it2 = v.begin();
